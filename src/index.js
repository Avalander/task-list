...
const view = sidebar$ => sidebar$.map(sidebar => div('.wrapper', [
	sidebar,
	main('#content', [
		button('.btn.btn-info.toggle-sidebar', { dataset: { toggle: 'sidebar' }}, 'Toggle sidebar'),
		div([
...
const app = sources => {
	const sidebar = makeSidebar(sources)

	const sinks = {
		DOM: view(sidebar.DOM),
	}
	return sinks
}
